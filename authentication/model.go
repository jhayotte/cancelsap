...
// SSOCredentials holds stock animation sso information keycloak server url
...
/* Used to change json tag from stock-animation_api to stock_animation_api
...
type UserClaims struct {
	...
	ResourceAccess    struct {
		StockAnimationAPI struct {
			Roles []string `json:"roles"`
		} `json:"stock_animation_api"`
...
/*
{
  ...
  "groups": [
    "/stock-animation/blanc-mesnil-id-2"
  ],
  "username": "acellard",
  "active": true
}
...
type UserClaimsInput struct {
	...
	ResourceAccess    struct {
		StockAnimationAPI struct {
			Roles []string `json:"roles"`
		} `json:"stock-animation_api"` // client name given by keycloak for stock animation api
	} `json:"resource_access,omitempty"`
	Groups []string `json:"groups,omitempty"`
	jwt.StandardClaims
}
